...
function Player(x, y, direction) {
	...
	this.inventory = [
		new Bitmap('img/gun.png', 512, 256),
...
function Map(size) {
	...
	this.skybox = new Bitmap('img/skybox.jpg', 300, 106);
	this.wallTexture = [new Bitmap('img/brickwall.jpg', 275, 183),new Bitmap('img/door.jpg', 504, 640)];
	this.floorTexture = new Bitmap('img/floor_texture.jpg', 1200,857);
	this.light = 10;
	this.objects = [];
}
...
Camera.prototype.drawSpriteColumn = function(player,map,column,columnProps,sprites) {
	...
	for(var i = 0; i < sprites.length; i++){
		...
		if(spriteIsInColumn){
			var sv = [Math.cos(sprite.angle),Math.sin(sprite.angle)];
			var pv = [sprite.x-player.x,sprite.y-player.y];
			var alp = (Math.acos((sv[0]*pv[0]+sv[1]*pv[1])/Math.sqrt(Math.pow(pv[0],2)+Math.pow(pv[1],2)))*2/CIRCLE)%1;
			alp = (isNaN(alp)?0:alp)
			//console.log(Math.floor(sprite.texture.length*alp))
			//console.log(sprite.texture.length)
			//console.log(alp);
			var imag = sprite.texture[Math.floor(sprite.texture.length*alp)]
			textureX = Math.floor( imag.image.width / sprite.render.numColumns * ( column - sprite.render.firstColumn ) );
			...
			imag.image.style.webkitFilter = 'brightness(' + brightness + '%)';
			imag.image.style.filter = 'brightness(' + brightness  + '%)';
			
			ctx.drawImage(imag.image, textureX, 0, 1, imag.height, left, sprite.render.top, width, sprite.render.height);
...
Camera.prototype.drawColumn = function(column, ray, angle, map) {
	...
	for (var s = ray.length - 1; s >= 0; s--) {
		var step = ray[s];
		wallTexture = map.wallTexture[map.textureMap[parseInt(step.x)+(32*parseInt(step.y))]];
...
var display = document.getElementById('display'),
	player = new Player(23.5, 23.5, Math.PI * 0.3),
	map = new Map(32),
	objects = new Objects(),
...
for(var mi = 0; mi < 32; mi++){
	for(var yu = 0; yu < 32; yu++){
		map.wallGrid[mi+(32*yu)]=((mi*yu + mi + yu)%7==0?1:0)
	}
}
map.textureMap = [];
for(var pio = 0; pio < 32*32;pio++){
	map.textureMap.push((pio%8)==3?1:0);
}
...
socket.on('objectdata', function (data) {
	...
	//if(Object.keys(data).length != Object.keys(clientObjects).length){
	for(var ch in clientObjects){
		if(!data[ch]){
			delete clientObjects[ch];
		}
	}
	//}
	for(var obj in data){
		var flag = clientObjects[data[obj].id]
		clientObjects[data[obj].id] = data[obj];
		//console.log(clientObjects)
		if(flag){
			return;
		}
		map.addObject({
			color: 'green',
			texture: [new Bitmap('img/zombie_pickup.png', 344, 698),new Bitmap('img/cowboy.png', 639, 1500)],
			height: .7,
			width: .25,
			floorOffset: 0,
			id: data[obj].id,
			logic: function(seconds){
				
				if(!clientObjects[this.id]){
					delete this;
					return;
				}
				//console.log(this);
				for(var prop in clientObjects[this.id]){
					this[prop] = clientObjects[this.id][prop];
				}
			},
			speed: .1,
			angle: data[obj].angle
		},data.x,data.y);
	}
    //
});
...
setInterval(function(){
	for(var dex = 0; dex < map.objects.length; dex++){
		if(clientObjects[map.objects[dex].id] == undefined){
			map.objects.splice(dex,1);
		}
	}
},1000);

setInterval(function(){
	console.log(controls.states.shooting)
	socket.emit('update', { 
		x: player.x,
		y: player.y,
		angle: player.direction,
		shooting: controls.states.shooting
	});
},100);
